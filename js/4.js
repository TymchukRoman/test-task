const mapper = (rules) => {
    return (dataItem) => {
        ...
        for (let [oldFieldName, newFieldName, transformer] of rules) {
            if (dataItem.hasOwnProperty(oldFieldName)) {
                transformed[newFieldName] = transformer ? transformer(dataItem[oldFieldName]) : dataItem[oldFieldName];
            }
        }
